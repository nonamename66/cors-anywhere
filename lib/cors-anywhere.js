...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    originBlockList: [],            // Requests from these origins will be blocked.
    originAllowList: ['xcoins.com','localbitcoins.com','exito.com'],            // If non-empty, requests not from an origin in this list will be blocked.
  ...
  return function(req, res) {
    ...
    if (!hasRequiredHeaders(req.headers)) {
      ...
      res.end('The header must be  "' + host + '"   to display');
      return;
    }

    var origin = req.headers.origin || '';
    if (corsAnywhere.originBlockList.indexOf(origin) >= 0) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + origin + '" was BLOCKLISTed by the operator of this proxy.');
      return;
    }

    if (corsAnywhere.originAllowList.length && corsAnywhere.originAllowList.indexOf(origin) === -1) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + origin + '" was not ALLOWLISTed by the operator of this proxy.');
      return;
    }
